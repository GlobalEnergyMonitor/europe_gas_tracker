...
CONFIG.default_title = 'Europe';

// default tilte for the thing we are mapping
CONFIG.fossil_name = 'fossil project';

// minzoom and maxzoom for the map
CONFIG.minzoom = 2;
CONFIG.maxzoom = 15;

// home bounds to focus on the "core area"
CONFIG.homebounds = [[31.5,-14],[65,45]];
...
CONFIG.attributes = {
  ...
  'units': {name: 'Capacity or Production units', format: 'string', classname: 'units', search: false, table: true, popup: false, tooltip: 'MW: Megawatts, MTPA: million tons per annum, boe/y: barrels of oil equivalent per year'},
  'start_year': {name: 'Start Year', format: 'string', classname: 'start_unit', search: false, table: true, popup: true},
  'pci5': {name: 'PCI 5', format: 'string', classname: 'pci5', search: false, table: false, popup: false},
  'pci6': {name: 'PCI 6', format: 'string', classname: 'pci6', search: false, table: false, popup: false},
...
function resetTheMap() {
  ...
  $('div.leaflet-control-layers-overlays input[value="pci-off"]').prop('checked', true); // select PCI-off (all projects)
...
function drawMap(filter=true) {
  ...
  let layers = L.geoJSON(DATA.filtered, {
    ...
    onEachFeature: function (feature, layer) {
      ...
      let dash = CONFIG.fossil_types[type]['dash'] ? CONFIG.fossil_types[type]['dash'] : 'none';
...
function filterData() {
  ...
  // get the "PCI" option
  let pci = $('div#pci-types input:radio:checked').val();
...
function searchMapForText() {
  ...
  $('div.leaflet-control-layers-overlays input[type="checkbox"]').prop('checked','checked'); // select everything in the legend
  $('div.leaflet-control-layers-overlays input[value="pci-off"]').prop('checked', true); // select PCI-off (all projects)
